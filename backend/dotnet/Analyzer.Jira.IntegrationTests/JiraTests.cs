...
namespace Analyzer.Jira.IntegrationTests
{
    public class JiraTests
    {
        ...
        {
            // var elasticService = new JiraElasticService()
            ...
            var res = client.Search<ChangeLog>(s => s.From(0)
        ...
        //.LessThan(new DateTime(2018, 01, 01))
            ...
            // TODO: ПЕрейти на JiraElasticService полностью от получения до мапинга 

            //var dash = new DashService(_jiraConfig);
            //var maped = dash.GetDash(res.ToList());
        }

        // TODO: Перейти на этот метод с пингом!
        ...
        {
            // Arrange
            var jiraService = new JiraService(new JiraLoader(_jiraConfig), new LoggerMock());

            // Act
            var result = await jiraService.GetJiraInfo(DateTimeOffset.Now.AddDays(-60), DateTimeOffset.Now);

            // new DashService(_jiraConfig)
            ...
            await client.DeleteByQueryAsync<object>(del => del
