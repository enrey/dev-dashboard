    ...
    public class GitLabService : IGitLabService
    {
        ...
        public async Task<IEnumerable<CommentsStatisicsDto>> GetMergeRequestsCommentsStatistics(DateTime startDate, DateTime endDate)
        {
            ...
            var result = usersActive.AsParallel()
                ...
                .Select(o => GetItemFromGitlab(o.Id, o.Username, o.Email, startDate, endDate).Result)
                .ToList();

            return result;
        }

        private async Task<CommentsStatisicsDto> GetItemFromGitlab(int userId, string username, string useremail, DateTime startDate, DateTime endDate)
        {
            ...
            {
                // оказывается коммент может быть привязан не к МР а например к коммиту
                if (a.target_type.ToString() == TYPE_COMMENT && a.note.noteable_iid != null)
                {
                    var item = new CommentsStatisicsItemDto
                    {
                        Dt = DateTime.Parse(a.note.created_at.ToString()),
                        Comment = a.note.body.ToString(),
                        MRTitle = a.target_title.ToString(),
                        MRId = int.Parse(a.note.noteable_iid.ToString()),
                        ProjectId = int.Parse(a.project_id.ToString()),
                    };
        ...
        public async Task<IEnumerable<UserMergeRequestsStatisicsDto>> GetMergeRequestsStatistics(DateTime startDate, DateTime endDate)
        {
            ...
            var creationData = mergeRequests
                ...
                .Select(gr => new
                {
                    ...
                    gr.Key.ProjectId,
                    Created = gr.Count(),
                    CreatedDates = gr.Select(x => (x.Iid, Dt: x.CreatedAt, x.Title, ProjectId: x.ProjectId)).ToList()
                })
                .ToList();
            ...
            var mergeData = mergeRequests
                ...
                .Select(gr => new
                {
                    ...
                    gr.Key.ProjectId,
                    Merged = gr.Count(),
                    MergedDates = gr.Select(x => (x.Iid, Dt: x.MergedAt.Value, x.Title, ProjectId: x.ProjectId)).ToList()
                })
                .ToList();
            ...
            var userProjectPairs = creationData
                .Select(cd => new
                {
                    cd.UserId,
                    cd.CreatedDates,
                    MergedDates = new List<(int Iid, DateTime Dt, string Title, string ProjectId)>(),
                    cd.ProjectId
                })
                .Concat(
                    ...
                    {
                        md.UserId,
                        CreatedDates = new List<(int Iid, DateTime Dt, string Title, string ProjectId)>(),
                        md.MergedDates,
                        md.ProjectId
                    })
                )
            ...
            var statistics = userProjectPairs
                ...
                .Select(o => new UserMergeRequestsStatisicsDto
                {
                    ...
                    OpenedDates = o.Select(o => o.OpenedDates).SelectMany(_ => _.CreatedDates.Select(i => new DateAndIdItem
                    {
                        Dt = i.Dt,
                        Url = _.Url + URL_SUFFIX + i.Iid,
                        Iid = i.Iid,
                        ProjectId = int.Parse(i.ProjectId),
                        Repo = _.Name,
                        Title = i.Title
                    })).ToList(),
                    MergedDates = o.Select(o => o.MergedDates).SelectMany(_ => _.MergedDates.Select(i => new DateAndIdItem
                    {
                        Dt = i.Dt,
                        Url = _.Url + URL_SUFFIX + i.Iid,
                        Iid = i.Iid,
